...
// 动画雷达图组件
export const AnimatedRadarChart = React.forwardRef(({ data, width = 400, height = 300, ...props }, ref) => {
  return (
    <AnimatedChart ref={ref} type="radar" {...props}>
...
// 动画柱状图组件
export const AnimatedBarChart = React.forwardRef(({ data, width = 400, height = 300, ...props }, ref) => {
  return (
    <AnimatedChart ref={ref} type="bar" {...props}>
  ...
  useEffect(() => {
    ...
    animationRef.current = gsap.fromTo(element,
      { textContent: 0 },
      {
        ...
        snap: { textContent: 1 },
        onUpdate: function () {
          element.textContent = format(this.targets()[0].textContent)
        }
      }
    )
...
AnimatedRadarChart.displayName = 'AnimatedRadarChart'

...
test.describe('Sangok Admin Authentication Test', () => {
  ...
  test('should require login for admin access', async ({ page }) => {
    console.log('🧪 Starting sangok admin authentication test...');
    
    // Step 1: Navigate to admin page
    console.log('📍 Step 1: Navigating to sangok admin page');
    await page.goto(ADMIN_URL);
    await page.waitForLoadState('networkidle');
    
    // Step 2: Verify we're on login page
    console.log('🔍 Step 2: Verifying initial state - should show login');
    const usernameInput = page.locator('input[label="用户名"], input[type="text"]').first();
    ...
    const adminTitle = page.locator('text=M-Profile Lab, text=管理后台, text=Modern Admin Panel').first();
    const logoutButton = page.locator('button[title="退出登录"], button:has-text("退出"), svg[data-testid="ExitToAppIcon"]').first();
    ...
    if (isLoggedIn) {
      ...
      await page.screenshot({ path: 'sangok-login-attempt.png' });
    }
    ...
    if (isLoginFormVisible) {
      ...
      await page.screenshot({ path: 'sangok-logout-state.png' });
    }
    ...
    const adminTitleAfterRefresh = page.locator('text=M-Profile Lab').first();
    ...
    await page.screenshot({ path: 'sangok-final-state.png' });
  ...
  test('should handle invalid admin session data', async ({ page }) => {
    console.log('🧪 Testing invalid admin session data handling for sangok...');
    ...
    if (isLoginFormVisible) {
      ...
      await page.screenshot({ path: 'sangok-invalid-session-test.png' });
    }
  });
});